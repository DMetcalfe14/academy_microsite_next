...
const Nav = () => {
  ...
  useEffect(() => {
    if (selectedIndex !== null) {
      ...
      input.setAttribute('aria-activedescendant', `suggestion-${selectedIndex}`);
    }
  }, [selectedIndex]);
  ...
  return (
    <nav className="bg-primary sticky top-0 w-full z-[100]" role="navigation" aria-label="Main navigation">
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          {/* Desktop Search */}
          <div className="hidden lg:flex flex-1 max-w-xl ml-8"> {/* Added spacing with ml-8 */}
            <form 
              id="search-form" 
            ...
            >
                ...
                >
                  {suggestions.map((course, index) => (
                    <div
                      key={index}
                      id={`suggestion-${index}`}
                      className={`p-2 cursor-pointer hover:bg-gray-100 ${selectedIndex === index ? 'bg-gray-100' : ''}`}
                      role="option"
                      aria-selected={selectedIndex === index}
                      onClick={() => handleSuggestionClick(course)}
                      onMouseEnter={() => setSelectedIndex(index)}
                      onKeyDown={(e) => e.key === 'Enter' && handleSuggestionClick(course)}
                      tabIndex={0}
                    >
                      {course.title}
                    </div>
              ...
              <Button type="submit" variant="white" className="flex-shrink-0">Search</Button>
            </form>
          </div>
